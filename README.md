...
- **Real-time AI responses** — streamed audio and transcript via OpenAI `gpt-4o-realtime-preview`
...
- **AI model** — `gpt-4o-realtime-preview` with `alloy` voice
- **Server** — Uvicorn (ASGI)
...
static/
  index.html      # Main UI (all JS inlined, no external dependencies)
...
The app is configured as an always-running VM deployment (required for persistent WebSocket connections).
